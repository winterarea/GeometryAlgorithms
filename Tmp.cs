...
namespace EarcutNet
{
    public class Earcut
    {
        public static List<int> Tessellate(IList<double> data, IList<int> holeIndices,int dim)
        {
        ...
        static Node LinkedList(IList<double> data, int start, int end, int dim,bool clockwise)
        {
        ...
        static void EarcutLinked(Node ear, IList<int> triangles, int dim, double minX, double minY, double invSize, int pass = 0)
        {
            if (ear == null)
            {
                return;
        ...
        static void SplitEarcut(Node start, IList<int> triangles, int dim,double minX, double minY, double invSize)
        {
        ...
        static Node EliminateHoles(IList<double> data, IList<int> holeIndices, Node outerNode,int dim)
        {
            ...
            for (var i = 0; i < queue.Count; i++)
            {
                ...
                Console.WriteLine(i + ":" + queue[i].x + "," + queue[i].y);
            }
        ...
        static Node EliminateHole(Node hole, Node outerNode)
        {
            ...
            if (bridge != null)
            {
                var b = SplitPolygon(bridge, hole);
                // filter collinear points around the cuts
                var f = FilterPoints(bridge, bridge.next);
                FilterPoints(b, b.next);
                return outerNode == b ? f : outerNode;
            }
            return outerNode;
        }
        ...
        static double SignedArea(IList<double> data, int start, int end,int dim)
        {
        ...
        public static double Deviation(IList<double> data, IList<int> holeIndices, int dim,IList<int> triangles)
        {
            var hasHoles = holeIndices.Count > 0;
            var outerLen = hasHoles ? holeIndices[0] * dim : data.Count;
